...
public class BinaryClassRelationshipJavaExtractionTest {
    ...
    public void testFieldVarLevelRelationMultiplicity() throws Exception {
        ...
        reqCon.insertFile(file2);
        reqCons.add(reqCon);
        final OOPSourceCodeModel codeModel = new ParsedProject(reqCon).model();        final ClassRelationshipsExtractor<?> bCAS = new ClassRelationshipsExtractor<Object>();
        final Map<String, BinaryClassRelationship> binaryRelationships = bCAS
                .generateBinaryClassRelationships(codeModel);
        final BinaryClassRelationship A_B = binaryRelationships.get("ClassA<<-->>ClassB");
        Assert.assertTrue("Multiciplicity from ClassA to ClassB should be '0..*'!", A_B.getbSideMultiplicity()
                .getValue()
                .equals(DefaultClassMultiplicities.ZEROTOMANY.getValue()));
        Assert.assertTrue("Multiciplicity from ClassB to ClassA should be '0..*'!", A_B.getaSideMultiplicity()
                .getValue().equals(DefaultClassMultiplicities.NONE.getValue()));
    }
    ...
    public void testInterfaceLevelRelationMultiplicities() throws Exception {
        ...
        final Map<String, BinaryClassRelationship> binaryRelationships = bCAS
                .generateBinaryClassRelationships(codeModel);
        final BinaryClassRelationship A_C = binaryRelationships.get("ClassA<<-->>ClassC");
        Assert.assertTrue("Multiciplicity from ClassA to InterfaceC should be '0..0'!", A_C.getaSideMultiplicity()
                .getValue().equals(DefaultClassMultiplicities.NONE.getValue()));
        Assert.assertTrue("Multiciplicity from InterfaceC to Class A should be '0..0'!", A_C.getbSideMultiplicity()
                .getValue().equals(DefaultClassMultiplicities.NONE.getValue()));
    }
    ...
    public void testExtensionLevelRelationMultiplicities() throws Exception {
        ...
        final Map<String, BinaryClassRelationship> binaryRelationships = bCAS
                .generateBinaryClassRelationships(codeModel);
        final BinaryClassRelationship A_D = binaryRelationships.get("ClassA<<-->>ClassD");
        Assert.assertTrue("Multiciplicity from ClassA to ClassD should be 'None'!", A_D.getaSideMultiplicity()
                .getValue().equals(DefaultClassMultiplicities.NONE.getValue()));
        Assert.assertTrue("Multiciplicity from ClassD to Class A should be 'None'!", A_D.getbSideMultiplicity()
                .getValue().equals(DefaultClassMultiplicities.NONE.getValue()));
    }
    ...
    public void testMethodReturnTypeLevelRelationMultiplicities() throws Exception {
        final RawFile file = new RawFile("ClassA.java", "package com.sample;" + "import java.util.ArrayList;"
                + "public class ClassA { ClassE aMethod() {}}");
        ...
        Assert.assertTrue("Multiciplicity from ClassA to ClassE should be 'None'!", A_E.getaSideMultiplicity()
                .getValue().equals(DefaultClassMultiplicities.NONE.getValue()));
        Assert.assertTrue("Multiciplicity from ClassD to Class A should be 'None'!", A_E.getbSideMultiplicity()
