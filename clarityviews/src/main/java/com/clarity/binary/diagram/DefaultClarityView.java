...
public class DefaultClarityView implements ClarityView, Serializable {
     ...
     * @param binaryRelationships A list of all the binary relationships in the code base.
     * @param model Source code model representing the entire code base.
     * @param diagramComponent The component for which the Clarity View is being generated.
     * @param callback Whether or not the diagram should be interactive.
     */
    public DefaultClarityView(ClassDiagramColorScheme colorScheme, Map<String, BinaryClassRelationship> binaryRelationships, 
    		OOPSourceCodeModel model, Component diagramComponent, boolean callback)
            throws Exception {
        ...
        for (final Map.Entry<String, Component> entry : model.getComponents().entrySet()) {
            ...
            if (cmp.componentType().isBaseComponent()) {
            	SvgGraphics.displayComponents.add(new ComponentDisplayInfo(new DiagramClassDisplayName(cmp.uniqueName()).value(),
            			cmp.uniqueName(), "#22df80", cmp.componentType().getValue()));
            } else if (cmp.componentType().isMethodComponent()) {
            	SvgGraphics.displayComponents.add(new ComponentDisplayInfo(new DiagramMethodDisplayName(cmp.uniqueName()).value(),
            			cmp.uniqueName(), "#C5C8C6", cmp.componentType().getValue()));
            } else if (cmp.componentType().isVariableComponent()) {
            	SvgGraphics.displayComponents.add(new ComponentDisplayInfo(cmp.name(),
            			cmp.uniqueName(), "#C5C8C6", cmp.componentType().getValue()));
            } 
        }
    ...
    public DefaultClarityView(Map<String, BinaryClassRelationship> binaryRelationships, OOPSourceCodeModel model, Component diagramComponent, boolean callback)
            throws Exception {        
