...
public class RelatedComponentsGroup {
     ...
     *            A list of components all other components in the group must
    ...
    public Set<Component> components() {
        ...
        for (String cmpName : mainComponents) {
            ...
            // Filter stage 3: get extends/implemented components of the newly
            // added
            ...
            // Filter stage 5: if there is space left, get any remaining even
            // weaker
            // relationships!
            for (int j = componentRelatedGroup.size() - 1; j > 0; j--) {
                for (final Map.Entry<String, BinaryClassRelationship> entry : allRelationships.entrySet()) {
            ...
            for (final Map.Entry<String, BinaryClassRelationship> entry : allRelationships.entrySet()) {
                ...
                } else if (cmpB.uniqueName().equals(cmp.uniqueName()) && !cmpA.uniqueName().equals(cmpB.uniqueName())
            ...
            for (final Map.Entry<String, BinaryClassRelationship> entry : allRelationships.entrySet()) {
                ...
                if (cmpA.uniqueName().equals(cmp.uniqueName()) && !cmpB.uniqueName().equals(cmpA.uniqueName())
                    ...
                    componentRelatedGroup.add(cmpB);
                } else if (cmpB.uniqueName().equals(cmp.uniqueName()) && !cmpB.uniqueName().equals(cmpA.uniqueName())
                        && !componentRelatedGroup.contains(cmpA)) {
                    componentRelatedGroup.add(cmpA);
                }
            }
