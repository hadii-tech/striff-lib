...
public class PlantUMLClassDiagramGenerator implements DiagramGenerator {
  ...
  /**
   * Generates a plantUML String describing the details of the classes contained in the
   * 'activeComponents' list.
   *
   * @param activeComponents The components under consideration for the current diagram.
   * @param componentList list of all the components in the code base
  ...
  private static String genClassDetails(final ArrayList<Component> activeComponents,
    ...
    for (final Component component : activeComponents) {
      ...
      if (component.componentType().isBaseComponent()) {
        ...
        tempStrBuilder.append(
            OOPSourceModelConstants.getJavaComponentTypes().get(component.componentType()) + " ");
        ...
        for (final String classChildCmpName : component.children()) {
          ...
          if ((component.componentType() == ComponentType.INTERFACE
            ...
            if ((childCmp != null) && !childCmp.componentType().isBaseComponent()) {
              ...
              } else if (childCmp.modifiers().contains(OOPSourceModelConstants
                  .getJavaAccessModifierMap().get(AccessModifiers.PRIVATE))) {
              ...
              if (childCmp.modifiers().contains(OOPSourceModelConstants.getJavaAccessModifierMap()
  ...
  /**
   * Generates a PlantUML string representing all the inter-class relationships between the given
   * activeComponents.
   *
   * @param activeComponents Components from which to search for class relationships.
   * @param binaryRelationships list of all the binary class relationships in the code base
   * @return PlantUML text description of the relations between the given class components.
   */
  private static String addClassRelations(final ArrayList<Component> activeComponents,
    ...
    for (final Map.Entry<String, BinaryClassRelationship> entry : binaryRelationships.entrySet()) {
      ...
      final String[] relationshipClassNames =
          entry.getKey().split(BinaryClassRelationship.getClassNameSplitter());
      final BinaryClassRelationship relationship = entry.getValue();

      if ((relationshipClassNames[0] != null) && ((relationshipClassNames[1] != null)
          && activeComponents.contains(relationship.getClassA())
          && activeComponents.contains(relationship.getClassB())
          && relationship.getClassA().componentType().isBaseComponent()
          && relationship.getClassB().componentType().isBaseComponent()
          && (relationship != null))) {
        ...
        if (!relationship.getbSideMultiplicity().getValue().isEmpty()
            && !relationship.getbSideMultiplicity().getValue()
                .equals(DefaultClassMultiplicities.ZEROTOONE.getValue())) {
        ...
        if (!relationship.getaSideMultiplicity().getValue().isEmpty()
            && !relationship.getaSideMultiplicity().getValue()
                .equals(DefaultClassMultiplicities.ZEROTOONE.getValue())) {
   ...
   *
   * @param componentGroup list of component groupings that form each diagram
  ...
  private byte[] genPlantUMLDiagrams(final ArrayList<Component> componentGroup,
      final Map<String, BinaryClassRelationship> binaryRelationships,
      final Map<String, Component> componentList) throws IOException, InterruptedException {
  ...
  public final String generateDiagram(final Component component,
      final Map<String, BinaryClassRelationship> binaryRelationships,
      final Map<String, Component> componentList, final int diagramSize)
      throws InterruptedException, IOException {

    final long startTime = new Date().getTime();
    final ArrayList<Component> componentGroups =
        new RelatedDiagramComponents(componentList, binaryRelationships, component, diagramSize)
            .components();
    final byte[] diagramString =
        genPlantUMLDiagrams(componentGroups, binaryRelationships, componentList);
    final String diagram = new String(diagramString);
    System.out.println(" Clarity View diagram generated in " + (new Date().getTime() - startTime)
        + " milliseconds.");
    return diagram;
  }
  ...
  /**
   *
   * @param theme color theme of the diagram
   * @param classDefinitions PlantUML String containing class definitions
   * @param classRelations PlantUML String containing class relationships
   * @return PlantUML string representing the finished diagram
   * @throws IOException Exception
   */
  private static String generatePlantUMLString(final String classDefinitions,
      final String classRelations) throws IOException {
