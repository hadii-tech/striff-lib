...
import com.hadii.striff.text.HtmlTagsStrippedText;
import com.hadii.striff.text.JavaDocSymbolStrippedText;
import com.hadii.striff.text.LineBreakedText;
import com.hadii.striff.text.SimplifiedJavaDocText;
...
final class PUMLClassFieldsDesc {
    ...
    PUMLClassFieldsDesc(Set<DiagramComponent> diagramComponents,
                        List<DiagramComponent> deletedComponents,
                        List<DiagramComponent> addedComponents, Map<String, DiagramComponent> allComponents, DiagramColorScheme colorScheme) {
    ...
    public String value() {
        ...
        for (final DiagramComponent component : diagramComponents) {
            ...
            if (component.componentType().isBaseComponent() && !component.uniqueName().contains("(")) {
                ...
                if (component.children().size() > MAX_ATTRIBUTE_SIZE) {
                ...
                if (hasChanged || component.children().size() <= MAX_ATTRIBUTE_SIZE) {
                    for (final String classChildCmpName : component.children()) {
                        ...
                        if (component.children().size() <= MAX_ATTRIBUTE_SIZE || addedComponents.contains(classChildCmpName)
                                || deletedComponents.contains(classChildCmpName)) {
                ...
                if (component.componentType() == OOPSourceModelConstants.ComponentType.INTERFACE
                        || component.modifiers().contains("abstract") || (!hasChanged && component.children().size() > MAX_ATTRIBUTE_SIZE)) {
    ...
    private void drawComponentComment(int longestLine, DiagramComponent component, List<String> componentPUMLStrings) {
        ...
        if (!commentStr.isEmpty()) {
            ...
            if (component.children().size() > 0 && component.children().size() < MAX_ATTRIBUTE_SIZE) {
    ...
    private boolean isChanged(DiagramComponent cmp) {
        if (addedComponents.contains(cmp) || deletedComponents.contains(cmp)) {
            return true;
        } else {
    ...
    private String colorClassBackground(DiagramComponent cmp, List<DiagramComponent> addedComponents2, List<DiagramComponent> deletedComponents2,
                                        String text) {
